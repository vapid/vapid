...
class Template {
   ...
   */
  static fromFile(filePath, partials = []) {
 ...
 * @private
 *
 * Maps array of partials to an object
 *
 * @param {array} paths - array of absolute paths
 * @return {Object} map of names to paths
 */
function _partialsMap(paths) {
...
function _replacePartials(html, partials) {
  ...
  const map = _partialsMap(partials);
  return html.replace(PARTIALS_REGEX, (match, name) => {
    const partial = map[name];
    return partial ? readFileSync(partial, 'utf-8') : '';
  });
}
